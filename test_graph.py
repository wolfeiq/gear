...
def build_test_graph():
    ...
    # Load test results (from test 2)
    try:
        with open('test_results.json', 'r') as f:
            data = json.load(f)
        print(f"✅ Loaded test_results.json ({len(data)} statements)\n")
    except FileNotFoundError:
        print("❌ test_results.json not found")
        print("Run: python test_full_extraction.py first")
        return
    ...
    print(f"✅ Saved graph data: test_graph_data.json")
    
    print("\n" + "="*60)
    print("✅ GRAPH TEST COMPLETE")
    print("="*60)
    print("\nAll test scripts passed! You're ready for:")
    print("  1. Full pipeline on Kaggle dataset")
    print("  2. Large-scale synthetic generation")
    print("  3. GeAR implementation")
